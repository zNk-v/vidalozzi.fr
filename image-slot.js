...
(() => {
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      // Always encode at MAX_DIM cap — independent of the slot's rendered
      // width — so click-to-zoom lightboxes get a crisp full-size image.
      const cap = MAX_DIM;
  ...
  const stylesheet =
    ...
    // Read-only gate: hide upload affordances when page isn't in edit mode
    ':host-context(html[data-editmode="0"]) .empty{display:none !important}' +
    ':host-context(html[data-editmode="0"]) .ctl{display:none !important}';
